...
module.exports = function (context) {
    ...
    if (plugins.length > 0 && plugins.indexOf('cordova-plugin-jb-jpushtunnel') === -1) {
        return ;
    }
    ...
    if (context.opts.cordova.platforms.indexOf("android") === -1) {
       ...
       var targetDir = path.join(projectRoot, "platforms", "android", "app","src","main","java","com","tunnel","plugin");  


        targetFiles = ["JPushReceiveActivity.java"];

    if (['after_plugin_add', 'after_plugin_install'].indexOf(context.hook) === -1) {
        // remove it?
          console.info("正在移除cordova-plugin-jb-jpushtunnel插件中...");
    } else {
        // sync the content
